...
function lib:RefreshZoneMeasurement(zoneIndex, zoneId)
	if self.zoneMeasurements[zoneId] then
		self.currentZoneMeasurement = lib.zoneMeasurements[zoneId]
		return 
	end
		...
		if success then
			...
			if firstX ~= secondX and firstY ~= secondY then
				...
				self.currentZoneMeasurement:Initialize(zoneIndex, zoneId,
						currentOriginGlobalX, currentOriginGlobalY, currentGlobalToWorldFactor)
			end
		end
	end
	...
	self.zoneMeasurements[zoneId] = self.currentZoneMeasurement
...
function lib:GetGlobalToWorldFactor(zoneId)
	...
	if zoneMeasurement then -- valid computation?
		return zoneMeasurement.globalToWorld, true
	end
...
function lib:GetWorldOriginAsGlobal(zoneId)
	local zoneMeasurement = self.zoneMeasurements[zoneId]
	if zoneMeasurement then -- valid computation?
		return zoneMeasurement.originGlobalX, zoneMeasurement.originGlobalY, true
	end
	if self.savedVars[zoneId] and self.savedVars[zoneId][1] then
		return self.savedVars[zoneId][2], self.savedVars[zoneId][3], false
	end
...
function lib:GetZoneMeasurementForZoneId(zoneId)
	...
	local globalToWorldFactor = self.SPECIAL_GLOBAL_TO_WORLD_FACTORS[zoneId] or lib.DEFAULT_GLOBAL_TO_WORLD_FACTOR
	local originGlobalX = nil
	local originGlobalY = nil
	if self.savedVars[zoneId] and self.savedVars[zoneId][1] then
		local globalToWorldFactor = self.savedVars[zoneId][1]
		local originGlobalX = self.savedVars[zoneId][2]
		local originGlobalY = self.savedVars[zoneId][3]
	end
	zoneMeasurement = self.ZoneMeasurement:New(GetZoneIndex(zoneId), zoneId,
						originGlobalX, originGlobalY, globalToWorldFactor)
...
function lib:IsValidZone()
	return (self.currentZoneMeasurement ~= nil)
end
